...
export const handler = async (event: Record<string, unknown>) => {
  try {
    ...
    if (!source || !target) {
      return { statusCode: 400, body: JSON.stringify({ error: "validation_error", message: "source and target are required" }) };
    }
    if (source === target) {
      return { statusCode: 400, body: JSON.stringify({ error: "validation_error", message: "source and target must be different" }) };
    }

    // Verify both nodes exist
    const [srcNode, tgtNode] = await Promise.all([getNode(source), getNode(target)]);
    if (!srcNode) return { statusCode: 404, body: JSON.stringify({ error: "not_found", message: `Node '${source}' not found` }) };
    if (!tgtNode) return { statusCode: 404, body: JSON.stringify({ error: "not_found", message: `Node '${target}' not found` }) };
    ...
    return { statusCode: 500, body: JSON.stringify({ error: "internal_error", message: "Failed to create edge" }) };
  }
};
